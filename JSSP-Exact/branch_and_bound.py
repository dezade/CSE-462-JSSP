...
def greedy(jobs, num_jobs, num_machines, priority="fifo"):
    ...
    for _ in range(total_ops):
        ...
        if best_j == -1:
            # Safety: should never happen if total_ops is correct
            break
    ...
    # Confirm all operations were scheduled
...
def branch_and_bound(jobs, num_jobs, num_machines,
    ...
    if init_lb >= best:
        return best   # greedy already matches lower bound; optimal or near-optimal
    ...
    while heap:
        ...
        # Terminal node: all jobs fully scheduled
        ...
        # Branch: schedule next operation of each unfinished job
        for j in range(num_jobs):
...
def solve_instance(tc, time_limit=60.0):
    """
    Solve one test case. Returns (makespan, elapsed_seconds, warnings).

    Size thresholds:
      n*m <= 225 (~15x15): full B&B, up to 500k nodes
      n*m <= 900 (~30x30): B&B with 100k node cap
      n*m  > 900          : best of 4 greedy heuristics only
    """
    ...
    # Step 1: greedy warm-start (always runs)
    ...
    # Step 3: optionally improve with B&B
    if size <= 225:
...
def process_results(optimal, raw_result):
    ...
    # If solver found a result better than known optimal, assume known optimal is correct
    if acc > 100.0 + 1e-6:
...
def main():
    ...
    for idx, tc in enumerate(test_cases, 1):
        ...
        try:
            raw_result, elapsed, warnings = solve_instance(tc, time_limit=time_limit)
        ...
        # Print warning if accuracy was anomalous (original beating optimal)
        warn_str = ""
        if optimal and raw_result < optimal - 1e-6:
            warn_str = f"  *** BEATS OPTIMAL: Result={raw_result} Optimal={optimal} (Corrected to 100%) ***"
        if warnings:
            warn_str += f"  WARN: {warnings[0]}"

        print(f"result={final_result}  accuracy={accuracy}%  time={elapsed}s{warn_str}")
    ...
    with open(output_csv, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["sample_no", "instance_id", "raw_result", "accuracy", "time_required"]
        )
        writer.writeheader()
    ...
    if numeric_acc:
        ...
        print(f"Accuracy  avg={avg}%  min={mn}%  max={mx}%  (over {len(numeric_acc)} instances)")
