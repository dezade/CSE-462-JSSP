...
\begin{frame}{Exact Algorithm: Branch \& Bound (B\&B)}
    ...
    \pause
    \vspace{0.5em}
    \textbf{Complexity}: Worst-case $O(2^{N_{disjunctive}})$. It guarantees an optimal solution but typically times out on densely constrained instances larger than $15 \times 15$.
\end{frame}
...
\begin{frame}{Heuristic: Most Work Remaining (MWKR)}
    \textbf{Concept}: A greedy Priority Dispatching Rule (PDR). When a machine is free, select the available operation whose job has the maximum total processing time remaining.
    ...
    \begin{algorithmic}[1]
        ...
        \While{Not all ops scheduled}
            ...
            \EndIf
            \State \textbf{Select} $O_{ij} \in A$ maximizing $\sum_{x=j}^{n_i} p_{ix}$ \textit{(MWKR Rule)}
